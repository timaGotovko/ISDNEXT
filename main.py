...
async def send_error(m: Message, where: str, exc: Exception):
    """
    Отправляем текст ошибки пользователю (с урезанным трейсбэком).
    """
...
def parse_booking_info(xml_text: str) -> dict:
    """
    Парсим Arrival(Start) / Departure(End) / GivenName / Surname / Phone / Email из XML.
    """
    if not xml_text:
        return {}
    try:
...
def write_hotel_excel(hotel_name: str, rows: list[dict], out_dir: Path) -> Path:
    """
    Excel с колонками: Departure (End), Arrival (Start), GivenName, Surname, Phone, Email.
    """
    ...
    cols = ["Departure", "Arrival", "GivenName", "Surname", "Phone", "Email"]
    for c in cols:
...
def build_hotel_reports(pms_to_name: dict[int, str], run_dir: Path) -> List[Path]:
    """
...
def create_zip(files: List[Path], archive_path: Path) -> Path:
    """ Создаём ZIP-архив из списка файлов. """
...
async def run_job_and_reply(m: Message, username: str, password: str, date_from: str, date_to: str, cm_code: str, run_dir: Path):
    try:
        ...
        # 3) Excel-отчёты
        try:
            reports = build_hotel_reports(pms_to_name, run_dir)
            if not reports:
        ...
        await m.answer(f"Сформировано отчётов: {len(reports)}. Упаковываю в ZIP...")

        # 4) Архив (ZIP) и отправка
        try:
            archive_path = run_dir / "reports"
            final_archive = create_zip(reports, archive_path)
            await m.answer_document(FSInputFile(final_archive), caption=f"Готово! Всего Excel: {len(reports)}.")
        except Exception as e:
        ...
        # Чистим все артефакты независимо от исхода
        try:
            safe_rmtree(run_dir)
            safe_rmtree(OLD_XML_DIR)
        except Exception:
